...
// collect the DOIs from the tag: and tags: fields in the front matter, like the layouts do
const dois = new Set()
for (const dir of ['_posts', 'tag', 'work']) {
  for (const file of fs.readdirSync(dir, { recursive: true })) {
    ...
    for (const tagLines of header.matchAll(/^tags?:.*(\n[ \t]+.*)*/gm)) {
      for (const tag of tagLines[0].replace(/^tags?:/, '').split(/\s+/)) {
